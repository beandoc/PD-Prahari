...
export default function DashboardLayout({
  ...
  const navLinks = isNursePath ? nurseNavLinks : doctorNavLinks;
  const isDashboardPage = pathname === '/dashboard' || pathname === '/dashboard/nurse-dashboard';
  const backLinkHref = isNursePath ? '/dashboard/nurse-dashboard' : '/dashboard';
  const userName = isNursePath ? 'Mr. Kamlesh' : 'Dr. Sachin';
    ...
    <div className="grid min-h-screen w-full md:grid-cols-[220px_1fr] lg:grid-cols-[280px_1fr]">
      ...
      <div className="flex flex-col">
        <header className="flex h-14 items-center gap-4 border-b bg-card px-4 lg:h-[60px] lg:px-6">
          <MobileNav navLinks={navLinks} pathname={pathname} userName={userName} />
